...
[gzhubenke 模板](gzhubenke.cls) 仅支持在XeLaTeX环境下编译，在pdflatex引擎下可能出现不允许使用fontspec的问题。

模板的中文字体优先交给 `ctex` 处理，默认使用 TeX Live 自带的 Fandol 字体。需要指定平台字体集时，可以使用 `ctex` 的 `fontset` 选项：

```Tex
\documentclass{gzhubenke}                  % 默认使用 TeX Live 自带的 Fandol 字体
\documentclass[fontset=fandol]{gzhubenke}  % 显式使用 Fandol 字体
\documentclass[fontset=windows]{gzhubenke} % 使用 ctex 的 Windows 字体集
\documentclass[fontset=mac]{gzhubenke}     % 使用 ctex 的 macOS 字体集
\documentclass[fontset=ubuntu]{gzhubenke}  % 使用 ctex 的 Ubuntu 字体集
\documentclass[fontset=none]{gzhubenke}    % 不让 ctex 自动设置中文字体
```

如果只需要覆写个别字体，可以在 `\documentclass` 后、`\begin{document}` 前使用：

```Tex
\gzhusetup{
  latin-main = Times New Roman,
  latin-sans = Times New Roman,
  latin-mono = Consolas,
  cjk-song = SimSun,
  cjk-hei = SimHei,
  cjk-kai = KaiTi,
...
对于Windows/MacOS/Ubuntu用户，系统一般配置了模板所需的黑体/宋体/楷体，但是由于不同字体在不同系统，甚至不同电脑上的不同，你可能需要通过 `fontset` 或 `\gzhusetup` 覆写字体名称以使xelatex能够正确加载系统字体。
...
在shell环境下使用`sh makev.sh`可以快速创建当前`main.pdf`的副本，并提交在references.bib，main.tex, gzhubenke.cls以及assets文件夹中的所有变动
